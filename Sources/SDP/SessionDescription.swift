...
func unmarshalProtocolVersion(lexer: inout Lexer) throws -> StateFn? {
    ...
    guard let version = UInt32(value) else {
        throw SdpError.errParseInt(value, "unmarshalProtocolVersion")
    }
    ...
    if version != 0 {
        throw SdpError.errSdpInvalidSyntax(value)
    }
...
func unmarshalOrigin(lexer: inout Lexer) throws -> StateFn? {
    ...
    if fields.count != 6 {
        throw SdpError.errSdpInvalidSyntax("`o=\(value)`")
    }

    guard let sessionId = UInt64(fields[1]) else {
        throw SdpError.errParseInt(fields[1], "field 1 of unmarshalOrigin")
    }
    guard let sessionVersion = UInt64(fields[2]) else {
        throw SdpError.errParseInt(fields[2], "field 2 of unmarshalOrigin")
    }

    // Set according to currently registered with IANA
    // https://tools.ietf.org/html/rfc4566#section-8.2.6
    if indexOf(element: fields[3], dataSet: ["IN"]) == nil {
        throw SdpError.errSdpInvalidValue(fields[3])
    }

    // Set according to currently registered with IANA
    // https://tools.ietf.org/html/rfc4566#section-8.2.7
    if indexOf(element: fields[4], dataSet: ["IP4", "IP6"]) == nil {
        throw SdpError.errSdpInvalidValue(fields[4])
    }
...
func unmarshalConnectionInformation(value: String) throws -> ConnectionInformation? {
    let fields = value.split(separator: " ", omittingEmptySubsequences: true).map { String($0) }
    if fields.count < 2 {
        throw SdpError.errSdpInvalidSyntax("`c=\(value)`")
    }

    // Set according to currently registered with IANA
    // https://tools.ietf.org/html/rfc4566#section-8.2.6
    if indexOf(element: fields[0], dataSet: ["IN"]) == nil {
        throw SdpError.errSdpInvalidValue(fields[0])
    }

    // Set according to currently registered with IANA
    // https://tools.ietf.org/html/rfc4566#section-8.2.7
    if indexOf(element: fields[1], dataSet: ["IP4", "IP6"]) == nil {
        throw SdpError.errSdpInvalidValue(fields[1])
    }
...
func unmarshalBandwidth(value: String) throws -> Bandwidth {
    ...
    if parts.count != 2 {
        throw SdpError.errSdpInvalidSyntax("`b=\(value)`")
    }
    ...
    if experimental {
        ...
        if indexOf(element: bandwidthType, dataSet: ["CT", "AS"]) == nil {
            throw SdpError.errSdpInvalidValue(bandwidthType)
        }
    }

    guard let bandwidth = UInt64(parts[1]) else {
        throw SdpError.errParseInt(parts[1], "\(value) of unmarshalBandwidth")
    }
...
func unmarshalTiming(lexer: inout Lexer) throws -> StateFn? {
    ...
    let fields = value.split(separator: " ", omittingEmptySubsequences: true).map { String($0) }
    if fields.count < 2 {
        throw SdpError.errSdpInvalidSyntax("`t=\(value)`")
    }

    guard let startTime = UInt64(fields[0]) else {
        throw SdpError.errParseInt(fields[0], "field 0 of unmarshalTiming")
    }
    guard let stopTime = UInt64(fields[1]) else {
        throw SdpError.errParseInt(fields[1], "field 1 of unmarshalTiming")
    }
...
func unmarshalRepeatTimes(lexer: inout Lexer) throws -> StateFn? {
    ...
    let fields = value.split(separator: " ", omittingEmptySubsequences: true).map { String($0) }
    if fields.count < 3 {
        throw SdpError.errSdpInvalidSyntax("`r=\(value)`")
    }

    if lexer.desc.timeDescriptions.isEmpty {
        throw SdpError.errSdpEmptyTimeDescription
    }
...
func unmarshalTimeZones(lexer: inout Lexer) throws -> StateFn? {
    ...
    for i in stride(from: 0, to: fields.count, by: 2) {
        guard let adjustmentTime = UInt64(fields[i]) else {
            throw SdpError.errParseInt(fields[i], "fileds[\(i)] of unmarshalTimeZones")
        }
...
func unmarshalMediaDescription(lexer: inout Lexer) throws -> StateFn? {
    ...
    let fields = value.split(separator: " ", omittingEmptySubsequences: true).map { String($0) }
    if fields.count < 4 {
        throw SdpError.errSdpInvalidSyntax("`m=\(value)`")
    }
    ...
    guard let portValue = Int(parts[0]) else {
        throw SdpError.errParseInt(parts[0], "parts[0] of unmarshalMediaDescription")
    }
    var portRange: Int? = nil
    if parts.count > 1 {
        guard let range = Int(parts[1]) else {
            throw SdpError.errParseInt(parts[1], "parts[1] of unmarshalMediaDescription")
        }
        portRange = range
    }
    ...
    for proto in fields[2].split(separator: "/").map({ String($0) }) {
        ...
        {
            throw SdpError.errSdpInvalidValue(fields[2])
        }
        protos.append(proto)
    }
...
func parseTimeUnits(value: String) throws -> Int64 {
    ...
    guard let parsedNum = Int64(num) else {
        throw SdpError.errSdpInvalidValue(value)
    }
