...
public struct ExtMap: Equatable {
    ...
    public static func unmarshal(line: String) throws -> ExtMap {
        ...
        if parts.count != 2 {
            throw SdpError.errParseExtMap(line)
        }
        ...
        if value < 1 || value > 246 {
            throw SdpError.errParseExtMap("\(valdir[0]) -- extmap key must be in the range 1-246")
        }
        ...
        if valdir.count == 2 {
            ...
            if direction == nil {
                throw SdpError.errParseExtMap("unknown direction from \(valdir[1])")
            }
        }
...
func parseRtpMap(rtpMap: String) throws -> Codec {
    ...
    if components.count != 2 {
        throw SdpError.errCodecNotFound
    }

    let ptSplit = components[0].split(separator: ":")
    if ptSplit.count != 2 {
        throw SdpError.errMissingColon
    }
    guard let payloadType = UInt8(ptSplit[1]) else {
        throw SdpError.errParseInt(String(ptSplit[1]), rtpMap)
    }
    ...
    let clockRate: UInt32 = try {
        if split.count > 1 {
            guard let clockRate = UInt32(split[1]) else {
                throw SdpError.errParseInt(String(split[1]), rtpMap)
            }
            return clockRate
        } else {
            return 0
...
func parseFmtp(fmtp: String) throws -> Codec {
    ...
    if components.count != 2 {
        throw SdpError.errMissingWhitespace
    }
    ...
    guard let payloadType = UInt8(split[1]) else {
        throw SdpError.errParseInt(String(split[1]), fmtp)
    }
...
func parseRtcpFb(rtcpFb: String) throws -> Codec {
    ...
    guard let payloadType = UInt8(ptSplit[1]) else {
        throw SdpError.errParseInt(String(ptSplit[1]), rtcpFb)
    }
